...
const MathEngine = {
  ...
  floor1() {
    ...
    if (type === 'add') {
      ...
      const wrongVal1 = (b - a === x) ? b - a + 3 : b - a;
      wrongOptions = [
        wrongVal1, // Subtract instead of divide
  ...
  floor5() {
    ...
    if (type === 'dist') {
      ...
      const wrongVal1 = (partialDistribute === x) ? partialDistribute + 3 : partialDistribute;
      const addInstead = x + (b * 2);

      wrongOptions = [
        wrongVal1, // Forgot to multiply constant by a
        addInstead,
        correct + 2
      ];
    } else {
  ...
  floor12() {
    let a, b;
    do {
      a = (Math.random() > 0.5 ? 1 : -1) * (Math.floor(Math.random() * 5) + 1);
      b = (Math.random() > 0.5 ? 1 : -1) * (Math.floor(Math.random() * 5) + 1);
  ...
  floor13() {
    ...
    let b, k;
    do {
      b = (Math.random() > 0.5 ? 1 : -1) * (Math.floor(Math.random() * 6) + 1); 
  ...
  floor16() {
    let a, b;
    do {
      a = (Math.random() > 0.5 ? 1 : -1) * (Math.floor(Math.random() * 4) + 1);
      b = (Math.random() > 0.5 ? 1 : -1) * (Math.floor(Math.random() * 4) + 1);
    } while (Math.abs(a) === Math.abs(b));
  ...
  floor19() {
    ...
    let a, b;
    do {
      a = Math.floor(Math.random() * 3) + 2; // 2..4 to prevent wrong3 === correct
...
const Game = {
  ...
  cacheDOM() {
    ...
    this.screens.leaderboard = document.getElementById('screen-leaderboard');
  ...
  bindEvents() {
    ...
    // Leaderboard
    this.btnLeaderboard = document.getElementById('btn-show-leaderboard');
    ...
    this.btnLeaveDungeon.addEventListener('click', () => {
      if (confirm("Retreat from the dungeon? Your run will end and your score will be saved.")) {
  ...
  startBattle(floor) {
    if (this.screens.start.classList.contains('active')) {
      // New run started from main menu
      this.gold = 0;
      this.level = 1;
  ...
  saveToLeaderboard() {
    ...
    let board = JSON.parse(localStorage.getItem('mq_leaderboard') || '[]');
    const entry = {
    ...
    board.push(entry);
    // Sort descending by gold, then by floor
    board.sort((a, b) => b.gold - a.gold || b.floor - a.floor);
    // Keep top 10
    board = board.slice(0, 10);
    
    localStorage.setItem('mq_leaderboard', JSON.stringify(board));
  },
  ...
  showLeaderboard() {
    ...
    if (board.length === 0) {
      this.leaderboardList.innerHTML = `<li class="empty-board">No scholars have braved the dungeon yet.</li>`;
      return;
    }
    ...
    board.forEach((entry, idx) => {
      const clsName = classNames[entry.classId] || 'Scholar';
      html += `
        <li class="leaderboard-entry">
          <div class="rank-name">
            <span class="rank">#${idx + 1}</span>
            <span class="name">${entry.name} <span style="font-size: 0.65rem; color: var(--text-muted); font-weight: normal;">(${clsName})</span></span>
          </div>
          <div class="score-info">
            <div class="score-gold">${entry.gold} Gold</div>
            <div class="score-floor">Reached Fl. ${entry.floor}</div>
          </div>
        </li>
      `;
    });
